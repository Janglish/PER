...
class Agent(nn.Module):
    ...
    def update_network(self, target_net, optimizer, steps_done, total_steps):
        if len(self.replay_buffer.buffer) < self.batch_size:
            return
        
        experiences, weights, indices = self.replay_buffer.sample(self.batch_size, steps_done, total_steps)
        ...
        optimizer.zero_grad()
        loss = F.smooth_l1_loss(weights * state_action_values, weights * expected_state_action_values)
        loss.backward()
        ...
        for i, idx in enumerate(indices):
            self.replay_buffer.priorities[idx] = (abs(delta[i].item()) + epsilon) ** alpha
